...
async function generateBadge(username) {
    ...
    for (let i in allBadges) {
        ...
        if (userProfile.badges.includes(allBadges[i])) {
            //badge.src = fs.readFileSync(`./assets/images/badges/${allBadges[i]}.png`);
            badge.src = fs.readFileSync(`./assets/images/status/online.png`);
        } else {
            badge.src = fs.readFileSync(`./assets/images/status/offline.png`);
            //badge.src = fs.readFileSync(`./assets/images/badges/d${allBadges[i]}.png`);
        }
